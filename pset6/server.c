...
char* indexes(const char* path)
{
     char* h = "index.html\0";
    char* p = "index.php\0";
    char* html_temp = malloc(strlen(path) * sizeof(h));
    char* php_temp = malloc(strlen(path) * sizeof(p));
    strcpy(html_temp, path);
    strcpy(php_temp, path);
   
    char* html_path = strcat(html_temp, "index.html\0");
    char* php_path = strcat(php_temp, "index.php\0");
    ...
    } else if (access(php_path, F_OK) == 0) {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    int idx = 0;
    int total = sizeof(BYTE);
    for (int c = fgetc(fp); c != EOF; c = fgetc(fp)) {
        buffer[idx] = c;
        buffer = realloc(buffer, total += sizeof(BYTE));
        idx++;
    }
     *content = buffer;
    *length = idx;
...
const char* lookup(const char* path)
{
    const char dot = '.';
    char* end;
    char* mtype[] = {"text/css", "text/html", "image/gif", "image/x-icon", "image/jpeg",
        "text/javascript", "text/x-php", "image/png"};
    char* ext[] = {".css", ".html", ".gif", ".ico", ".jpg", ".js", ".php", ".png"};
    
    // variable to hold the .___ for the type of file
    end = strrchr(path, dot);
    //printf("end = %s", end);
    // loops through extensions to find a match with end
    for (int i = 0; i < 8; i++) {
        if (strcasecmp(end, ext[i]) == 0) {
            //printf("ext[i] = %s", ext[i]);
            return mtype[i];
        }
        else continue;
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* sp = " ";
    int k = 0;
    char* hold = malloc(strlen(copy) + 1);
    for (int i=0; i<strlen(copy);i++) {
        hold[i] = copy[i];
        if (strcmp(&hold[i], sp) == 0) {
            printf("space bros");
            k++;
        }
        if (k>2) {
    ...
    char* method = strtok(copy, sp);
    char* request_target = strtok(NULL, sp);
    char* version = strtok(NULL, sp);
     char* abs_qu;
    char* querycpy;
    abs_qu = strtok(request_target, "?");
    querycpy = strtok(NULL, "\0");
    if (querycpy != NULL)
    {
        strcpy(abs_path, abs_qu);
        strcpy(query, querycpy);
 
        abs_path[strlen(abs_qu)] = '\0';
        query[strlen(querycpy)] = '\0';
    }
    else
    {
        strcpy(abs_path, abs_qu);
